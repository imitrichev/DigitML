...
int main() {
        ...
        for(int i=0; i<COUNT_ALPHA; ++i)
        {
                ...
                n[i].train(num_iterations, images_train, labels_train);

                const double accuracy_train = calculate_accuracy(images_train, labels_train, n[i]);
                const double accuracy_test = calculate_accuracy(images_test, labels_test, n[i]);
